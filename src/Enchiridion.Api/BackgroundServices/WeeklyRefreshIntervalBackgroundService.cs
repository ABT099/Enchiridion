...
public class WeeklyRefreshIntervalBackgroundService(IServiceProvider services) : BackgroundService
{
    private static readonly Func<AppDbContext, DateTime, CancellationToken, Task> BatchHabitUpdate =
        EF.CompileAsyncQuery((AppDbContext db, DateTime nextRun, CancellationToken stoppingToken) =>
            db.Habits
                .Where(x => x.HabitOptions.RepeatInterval == RepeatInterval.Weekly &&
                            x.HabitOptions.TargetDate == DateTime.Today.AddDays(7) &&
        ...
        EF.CompileAsyncQuery((AppDbContext db, DateTime nextRun, CancellationToken stoppingToken) =>
            db.Todos.Where(
                    x => x.TodoOptions != null && 
                         x.TodoOptions.RepeatInterval == RepeatInterval.Weekly &&
                         x.TodoOptions.TargetDate ==  DateTime.Today.AddDays(7) &&
                         x.isRepeated == true)
                .ExecuteUpdateAsync(setters => setters
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            var nextRun = DateTime.UtcNow.AddDays(7);
