...
public class MonthlyRefreshIntervalBackgroundService(IServiceProvider services) : BackgroundService
{
    private static readonly Func<AppDbContext, DateTime, CancellationToken, Task> BatchHabitUpdate =
        EF.CompileAsyncQuery((AppDbContext db, DateTime nextRun, CancellationToken stoppingToken) =>
            db.Habits
                .Where(x => x.HabitOptions.RepeatInterval == RepeatInterval.Monthly &&
                            x.HabitOptions.TargetDate == DateTime.Today.AddMonths(1) &&
        ...
        EF.CompileAsyncQuery((AppDbContext db, DateTime nextRun, CancellationToken stoppingToken) =>
            db.Todos.Where(
                    x => x.TodoOptions != null && 
                         x.TodoOptions.RepeatInterval == RepeatInterval.Monthly &&
                         x.TodoOptions.TargetDate ==  DateTime.Today.AddMonths(1) &&
                         x.isRepeated == true)
                .ExecuteUpdateAsync(setters => setters
